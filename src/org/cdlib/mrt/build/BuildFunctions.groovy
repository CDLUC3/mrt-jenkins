...
def build_war(repo, mvnparams) {
  script {   
    ...
    sh "mvn -Dmaven.repo.local=${env.M2DIR} -s ${MAVEN_HOME}/conf/settings.xml clean install -Ddocker.imagePullPolicy=Always -Ddocker.imagePullPolicy.run=Always ${mvnparams}"
  }
}
